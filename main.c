...
/**
  ...
  * @attention
  *
  * Copyrighand then merge them using another video editing app (e.g., CapCut or Clipchamp) before uploading the final video to YouTube?t (c) 2026 STMicroelectronics.
  * All rights reserved.
  *
...
int main(void)
{
  ...
  HAL_TIM_Base_Start_IT(&htim1);  /*TIM: 10ms IRQ */
  HAL_TIM_Base_Start_IT(&htim3);  /*TIM: 100ms IRQ */
  HAL_TIM_Base_Start_IT(&htim2); //counter after 1 us

  /*start interrupt on receiving the data*/
  HAL_UART_Receive_IT(&huart1,&rx_byte,1);

  EV_Init(&ev); //ev initialization
  HCSR04_Init();//ultrasonic initialization
  ADAS_Init(&adas); // initial l r f 400 cm
  Fault_Init(&flt); //flag active=0 flag->none
  ...
  /* USER CODE BEGIN WHILE */
	  ...
	  while (1)
	  {
	      ...
	      if(sensor_flag==1)//100ms tick
	      {
	    	  ...
	    	  //update TTC,blind spot detection
	    	  ADAS_Update(&adas,&ev);

	    	  /*validating motor temp,SOC and collision update fault flag*/
	    	  Fault_Check(&flt, &ev, &adas);

	    	  //waiting to receive the data and process the command
	    	  Shell_Process();
